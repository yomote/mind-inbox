...
def test_単体_つなぎの長さ制限を検査できない範囲として明示する() -> None:
    ...
    無いと何が静かに通るか:
        つなぎの上限 (14 文字) を超える引用は**1 行も検査されない**のに、
        レポートは 0 件と出る。上限を広げれば拾えるが**誤検出が増える** (実測:
        24 文字で、引用ではなく書き手自身の言葉を括っただけの箇所を 3 件拾った) —
        どちらを選ぶにせよ、**選んだ結果として見ていない範囲**は毎回出す。
    """
    quoted = _Q_OPEN + "遠くにある文言" + _Q_CLOSE
    # 14 文字以内なら拾う / 超えると拾わない (境界そのものを固定する)
    assert iter_md_references("CLAUDE.md" + "あ" * 14 + quoted) != []
    assert iter_md_references("CLAUDE.md" + "あ" * 15 + quoted) == []
    assert any("14 文字より離れている" in item for item in UNCOVERED), (
        "長さ制限で検査できない範囲を UNCOVERED に書いていない (silent caps)"
    )
