...
def judge_zip(symlink_entries: list[str]) -> list[str]:
    ...
    保証しないこと (#424): 0 件は「ツリーに symlink が無かった」証拠では**ない**。
    現在の `zip -qr` は `-y` が無いので symlink を辿って実体を格納する (symlink 入り
    ツリーを同じフラグで固めても 0 件。実測)。symlink が無いことを保証しているのは
    tree 側の検査。

    いつ発火しうるか: **単独では発火しない**。次の 2 つが重なったときだけ 1 件以上になる。

    (a) アーカイバが実体化をやめる — `-y` の追加 / 別実装への差し替え、かつ
    (b) tree 検査が守っていない範囲に symlink が居る — tree 検査は `node_modules` しか
        歩かないので `dist/` 配下、または tree 検査が消された / 迂回された `node_modules`。

    (a) だけでは tree 検査が `node_modules` の symlink 0 本を保証済みなので 0 件のまま。
    つまりこの検査の実体は「tree 検査の二重化 + tree 検査が歩かない `dist/` 側の網」で、
    平常時に 0 件であること自体は正常。

    無いとこれが静かに通る: (a)(b) が揃った zip が、ツリー側だけ緑のまま Functions 上で
    require を解決できない状態で配信される。

    なお `dist/` 配下のリンク切れ symlink は現行フラグでは警告なく zip から落ちる
    (実測: exit 0 でエントリごと消える) — この検査にも tree 検査にも映らない。
    """
