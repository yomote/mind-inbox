...
# タイプ → アイコン PNG の唯一の真実 (#478)。viz-structure.sh が --argjson で jq に渡し、
# icon_warnings が「出せないアイコン」の判定に使う。PNG の実体は icons/ (無いものは
# icons/README.md の「未入手のアイコン」に記載)
ICON_MAP = {
...
def icon_warnings(nodes, available_icons):
    """アイコンを出せないノードの警告行を返す純粋関数 (#478)。

    無いと何が静かに通るか: ICON_MAP に無い種別や PNG 未配置の劣化が生成ログに
    出ず、素の箱だらけの図が公開 docs を上書きしても誰も気づけない (2026-08-09 の
    アイコン全滅事故と同じ経路)。判定を viz-structure.sh の jq に埋めず
    ここに置くのは、pytest で押さえるため (cicd/CLAUDE.md)。
    """
    ...
    for n in nodes:
        if n.get("external"):
            continue  # ブラウザ擬似ノードなど Azure 外はアイコン対象外
        t = lc(n.get("type"))
        if not t or t in seen:
            continue
        seen.add(t)
        mapped = ICON_MAP.get(t)
        if mapped is None:
            warnings.append(f"アイコン未登録: {n.get('type')} (素の箱で描画)")
        elif mapped not in available:
            warnings.append(f"アイコン PNG 未配置: {mapped} ({n.get('type')} は素の箱で描画)")
    return warnings
...
NAME_OVERRIDES = [
    ...
    # Observability (#478): appi は BFF 専用の受け皿。cicd/modules/bootstrap-core.bicep が
    # APPLICATIONINSIGHTS_CONNECTION_STRING を Functions ホストに渡し、ホストの自動収集が
    # requests/dependencies を送る (#307 / ADR 0055)。AI Agent は exporter を配線して
    # いない (#463 で固定 — app/observability.py 参照) ので「OTel の出口」ではない
    ("microsoft.insights/components", "appi-", "Observability (Application Insights, BFF only)",
        "Telemetry sink for the BFF Function App: the Functions host auto-collects "
        "requests/dependencies via APPLICATIONINSIGHTS_CONNECTION_STRING (#307 / ADR 0055); "
        "provisioned by enableAppInsights (default true) in cicd/iac/main-bootstrap.bicep, "
        "workspace-based — data lands in the central Log Analytics workspace. "
        "The AI Agent wires no exporter here (#463)"),
    ("microsoft.insights/actiongroups", "smart detection", "App Insights auto-created alert group",
        "Smart Detection notification group Azure provisions automatically alongside "
        "Application Insights; not declared in IaC and not referenced by app code"),
