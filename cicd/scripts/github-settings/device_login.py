...
この実装が守ること:
  ...
  - **黙って待たない**。コードが取れなければ即座に非ゼロで落ちる。
    ポーリング中も残り時間を定期的に出す (沈黙と正常を区別する / CLAUDE.md)
  - **トークンをログに出さない**。受け取った瞬間に `::add-mask::` へ渡し、
    ファイルには 0600 で書く
...
# 差し替えたいときのために workflow から --client-id で上書きできる。
...
def parse_device_code(payload: object) -> DeviceCode:
    ...
    if not isinstance(payload, dict):
        raise DeviceFlowError(f"応答が JSON オブジェクトではない: {payload!r}")
    ...
    if missing:
        raise DeviceFlowError(f"応答に必須項目が無い: {', '.join(missing)} / 実際: {sorted(payload)}")
    return DeviceCode(
...
def classify_poll(payload: object) -> PollResult:
    ...
    **未知の error を「まだ承認されていない」に丸めない** — 丸めると、
    ...
    """
    if not isinstance(payload, dict):
        return PollResult(kind="fail", detail=f"応答が JSON オブジェクトではない: {payload!r}")
...
def notice_lines(code: DeviceCode) -> list[str]:
    ...
    """
    return [
        f"::notice title=ワンタイムコード::{code.user_code}",
        f"::notice title=入力先::{code.verification_uri} を開いて上のコードを入力し、承認してください",
...
def request_json(url: str, fields: dict[str, str], timeout: int = 20) -> object:
    ...
    try:
        ...
        raise DeviceFlowError(f"{url} が HTTP {exc.code} を返した: {exc.read().decode()[:200]}") from exc
    except OSError as exc:
        raise DeviceFlowError(f"{url} に到達できない: {exc}") from exc
    try:
        return json.loads(body)
    except json.JSONDecodeError as exc:
        raise DeviceFlowError(f"{url} の応答が JSON ではない: {body[:200]}") from exc
...
def run(args: argparse.Namespace) -> int:
    ...
    code = parse_device_code(
        request_json(
            DEVICE_CODE_URL,
            {"client_id": args.client_id, "scope": args.scopes},
        )
    )
    ...
    while True:
        ...
        if result.kind == "ok":
            ...
            _write_token(args.token_out, result.token)
            emit("認証できました (トークンはこの run の中だけに存在します)")
            return 0
        if result.kind == "fail":
...
def _write_token(path: str, token: str) -> None:
    """トークンを 0600 で書く。環境変数や GITHUB_OUTPUT には出さない。
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--client-id", default=os.environ.get("GH_OAUTH_CLIENT_ID") or GH_CLI_CLIENT_ID)
    parser.add_argument("--scopes", default="repo,read:org")
    ...
    try:
        ...
        # コードが出せない = この先ぜんぶ無意味なので、待たずにここで落とす。
        emit(f"::error::{exc}")
