...
| [`check_key_vault.py`](check_key_vault.py)     | Key Vault の data-plane にまつわる判定 2 つ — 実行者の権限 (足りないロールを名指し / 権限以外の失敗は「未検証」で分ける) と、鍵が**エクスポート不可**か (**空が正常**)。同じく**シェルに埋めない**ための純粋関数 ([#499](https://github.com/yomote/mind-inbox/issues/499))                                                                                                          |
| [`test_bootstrap.py`](test_bootstrap.py)       | az / terraform / curl をスタブして全分岐を実測 (**pem 本文と installation token が出力・argv に現れないこと**を固定) + 判定モジュールを直接叩く単体。**az スタブは実応答の JSON を持ち、スクリプトが渡した `--query` を実際に評価する** — 期待値を echo する作りに戻すと JMESPath の誤りが構造的に検出できなくなる ([#499](https://github.com/yomote/mind-inbox/issues/499) 発見 3) |
