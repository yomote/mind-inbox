"""[単体] README の宣言 (エンドポイント表 / 環境変数表) を実装と機械照合する (#487)。
...
# 下の対称チェックが赤にするので、消し忘れたまま残ることはない。
...
def test_単体_README_のエンドポイント表が_FastAPI_の_routes_と一致する() -> None:
...
def test_単体_README_の環境変数表が_Settings_のフィールドと一致する() -> None:
    """無いと何が静かに通るか: Settings にフィールドを足しても README が古いまま緑で通る
    (逆に、消したフィールドの行が README に残っても緑で通る)。"""
    rows = _table_rows(_section(README.read_text(encoding="utf-8"), "環境変数"))
    readme_vars = set()
    for cells in rows:
