...
class TestOrganize:
    async def test_l1_maps_kernel_response_to_response_schema(
    ...
    async def test_json_inside_markdown_fence(self, session_repo, make_kernel):
        history = ChatHistory()
    ...
    async def test_malformed_json_returns_fallback(self, session_repo, make_kernel):
    ...
    async def test_missing_session_raises_value_error(self, session_repo, make_kernel):
        kernel = make_kernel("{}")

        with pytest.raises(ValueError, match="Session not found"):
            await organize("nonexistent", session_repo, kernel)

    async def test_partial_json_uses_defaults(self, session_repo, make_kernel):
        history = ChatHistory()
        history.add_user_message("テスト")
        await session_repo.save("s1", history)

        kernel = make_kernel(json.dumps({"summary": "部分的な要約"}))
