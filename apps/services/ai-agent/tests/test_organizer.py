...
class TestOrganize:
    async def test_l1_maps_client_response_to_response_schema(
        ...
        result = await organize("s1", session_repo, client_mock)
    ...
    async def test_json_inside_markdown_fence(self, session_repo, make_client):
        history = ChatHistory()
    ...
    async def test_malformed_json_returns_fallback(self, session_repo, make_client):
    ...
    async def test_missing_session_raises_value_error(self, session_repo, make_client):
        client_mock = make_client("{}")

        with pytest.raises(ValueError, match="Session not found"):
            await organize("nonexistent", session_repo, client_mock)

    async def test_partial_json_uses_defaults(self, session_repo, make_client):
        history = ChatHistory()
        history.add_user_message("テスト")
        await session_repo.save("s1", history)

        client_mock = make_client(json.dumps({"summary": "部分的な要約"}))
