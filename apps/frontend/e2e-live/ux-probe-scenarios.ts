 ...
 *   - 否定はどちらも**自分の前発話を言い直す**形で書く (「さっきの自分の言い方も違う」)。
 *     assistant の発話内容 (仮説・助言) を前提にする語 (「誰かのせいに…」等) を置くと、
 *     AI が仮説を出さず開いた質問だけを返した朝に「存在しない主張への否定」になって
 *     台本として壊れる (Codex P2 / PR #443)。機械判定は ux-probe-scenarios.test.ts
...
const HYPOTHESIS_PUSHBACK_V1: ProbeScenario = {
  ...
  userTurns: [
    ...
    "さっきの自分の言い方も少し違う気がしてきました。時間が戻ってこないというより、予定が埋まっていること自体が苦しいんだと思います。この感じとはどう付き合っていけばいいんでしょうか",
  ],
};
