 ...
 * 二重送信も「レコードが無い」も 404 で返していたため、BFF もフロントも
 * **副作用が実行されたかどうかを判定できなかった** (承認 status の保存は resume 実行の
 * 前に起きるので、実行済みの再送も 404 になる)。型を分け `status` を運ぶことで、
 * フロントは「すでに承認済み (= 実行された)」「すでに却下済み (= 実行されていない)」を
 * 言い切れる。
 *
 * `processedAt` は null がありうる (時刻を持たない古いレコード)。**判定に使わない** —
 * 時刻の欠落は「未処理」を意味しない。
 */
...
export async function approve(req: ApproveRequest): Promise<ApproveResponse> {
  ...
  return await trackDependency(
    ...
    async () => {
      ...
      // 上げることで、フロントは「すでに承認済み (= 実行された)」まで言い切れる。
      //
      // **すべての 409 を変換しない** — 404 側と同じ規律。承認と無関係な 409
      // (プロキシ / 別レイヤ / 将来の別ルート) を「処理済み」に写すと、生きている
      // 承認カードが「もう実行されました」の顔をして閉じる。判別は body の形で行う
