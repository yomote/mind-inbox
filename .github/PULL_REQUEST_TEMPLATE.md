...
<!-- ADR 0017。自動テストが緑でも「普通に動かせば見つかる」層は拾えない。実際に動かした結果を貼る -->

- **何を叩いて、何が返ったか** (出力を貼る。「設定した」ではなく**振る舞い**を書く): <!-- 例: `curl -o /dev/null -w '%{http_code}' https://func-.../api/trpc/health.ping` → `401` (EasyAuth が未認証を拒否) -->
- **UI 変更がある場合**: ローカル (mock + 認証なし) で確認したか / スクリーンショット
- **検証できなかったこと + 人の目が必要な箇所**: <!-- 例: MSAL の実ログイン往復は自動化できないため未確認。マージ後にブラウザで要確認 -->
...
- [ ] **動作検証をして結果を貼った** (「設定した」ではなく振る舞い) / 実施できない理由を書いた
- [ ] UI 変更があるなら**ブラウザで確認した** (ローカル mock + 認証なしで可) / 不要
