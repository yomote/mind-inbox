...
- **載せてよいのは PO 裁定が要る 3 種類だけ** — 不可逆・高コスト / 北極星 / PO 名義が要るもの (ADR の Accept・ux-rubric の改定)。**可逆な運用判断** (Issue の整理、実装方式、着手順、レビュー指摘への裁定、PR の受け入れ) は**窓口 PM が決めて事後報告するもの**で、ここに積んではいけない ([`docs/team.md`](../../../docs/team.md) の「委任の境界」/ 2026-08-15 PO 裁定)。**積むと裁定 3 枠を食い潰し、本当にレーンを止めているものが 4 件目に落ちて「寝かせ中 n 件」に沈む**
  - **例外 — レビュー指摘のうち①なのは「対応方針を選ぶ」ところまで。** 直す / 別 Issue に切る / 見送る、の選択は窓口 PM が決めてよいが、**見送りを選んで指摘者が再レビューで反対したとき、その反対を越えて resolve してよいか**は②に戻る ([`merge` skill](../merge/SKILL.md) Step 2 の例外条件 — 判定者を欠いたまま自分の宿題に合格を出さないための機構)。**ここを一律除外にすると、PM が見送りを選んだ PR が「🙋 あなたの番: なし」のまま止まり続ける**
...
- ❌ 窓口 PM が決めてよい可逆な運用判断を「🙋 あなたの番」に積む（裁定枠は 3 件しかない。決めて事後報告する側）
