...
> **このコマンドは [`docs/adr/README.md`](../../../docs/adr/README.md) 「1. 番号を決める」の写し。** 採番手順の正典はあちらなので、変えるときは**同じ PR で両方**揃える。
...
- 既に同じ番号が open PR にある → **guard は捕まえない。** guard が見るのは「base に既にある番号」と「退役番号の再利用」だけで、**他の open PR は視野に入らない** (`base.sha` としか比較せず、strict OFF なので古い緑のままマージできる)。**並行採番は、書く瞬間に `origin/main` から取ることでしか防げない**。衝突に気づいたら**自分が後発なら自分が動く** (最大 +1 を取り直す)
