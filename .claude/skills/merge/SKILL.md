...
- ⚠️ **反射的に main を追随しない。** 引き継ぎ判定は実装差分の指紋に patch 本文まで畳むため、**base が進んだだけの追随でも失効することがある** ([#323](https://github.com/yomote/mind-inbox/issues/323) が open / 2026-08-12 に PR #286 で実測)。ブランチ保護の strict (Require branches to be up to date) は **OFF** なので遅れたままマージできる。**追随するのはコンフリクトが実際に出たときだけ** (そのときは PR 側の内容も変わるので、再受け入れが要るのが正しい)
