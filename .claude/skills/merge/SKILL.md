...
- **操作は PM、判定は指摘者。** 修正せず見送る場合 (別 Issue へ切り出し等) に再レビューでも再提起されたら、独断で畳まず PO に上げる
  - **指摘そのものへの裁定 (直す / 別 Issue に切る / 見送る) は窓口 PM が聞かずに決めてよい** — 可逆な運用判断 ([`docs/team.md`](../../../docs/team.md) の「委任の境界」①)。PO に上がるのは**指摘者の反対を越えて resolve するか**だけで、これは判断の委任ではなく**判定者を欠いたまま自分の宿題に合格を出さないための機構**。上げ方も②と同じ (案 + 推奨をつけてまとめて出す。その PR が止まるだけで他は進む)
...
- **引き継ぎ (carryover) は当てにしない。** 規約上は、受け入れ SHA から現 head までの追加コミットが **base (main) からのマージのみ**で、かつ **PR の実装差分 (`base...head`) が受け入れ時点と同一**なら受け入れは現 head に引き継がれる。ただし **#323 が open の間、成立を前提に段取りを組まないこと** — 追随したら再受け入れが要る前提で動く (下の ⚠️)。実装差分が 1 文字でも変わる push は当然に再受け入れ
