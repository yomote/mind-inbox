...
- **deploy の前は `release-gate` skill を通す** — 実装セッション自身に Go/No-Go を判定させず、独立 judge 3 役 (いずれも新品コンテキストの subagent) に判定させる ([ADR 0015](docs/adr/0015-independent-judge-agents-security-qa-release.md) / [Runbook](docs/runbooks/review-agents.md))。security-reviewer は脆弱性スキャンツール併用、qa-reviewer は受け入れテスト (L3 E2E) の作成・実行まで担い (L3 の所有者)、release-judge が 3 レポート (開発/QA/セキュリティ) を突合して判定。審査基準は `.github/claude/{security,qa,release}-rubric.md` (rubric-as-truth)。🟢 でも deploy ボタンは人間
