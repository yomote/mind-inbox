...
- **自動化の生死**: [`docs/runbooks/automation-inventory.md`](docs/runbooks/automation-inventory.md) — CI / Routine / hook が**今どれが動いているか**の唯一の台帳。**自動化を足したら 1 行足す。足せないなら作らない**。新設の必須条件は「動いたら痕跡がリポジトリに残ること」(異常時だけ喋る設計にしない — 沈黙と正常が区別できなくなる)
