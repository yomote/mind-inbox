...
- **deploy の前は `release-gate` skill を通す** — 実装セッション自身に Go/No-Go を判定させず、独立 judge 3 役 (security-reviewer / qa-reviewer / release-judge、いずれも新品コンテキストの subagent) に判定させる ([ADR 0015](docs/adr/0015-independent-judge-agents-security-qa-release.md) / [Runbook](docs/runbooks/review-agents.md))。審査基準は `.github/claude/{security,qa,release}-rubric.md` (rubric-as-truth)。🟢 でも deploy ボタンは人間
