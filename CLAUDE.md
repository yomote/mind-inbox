...
- **分配の基準は「作業の大きさ」と「往復が要るか」** ([ADR 0048](docs/adr/0048-child-sessions-are-usable-again-with-a-one-way-poke-channel.md) が [ADR 0033](docs/adr/0033-parent-implements-via-subagent-when-child-sessions-are-gated.md) D1 を置き換え / 2026-08-12 実測) — **レビュー指摘への対応・1〜2 ファイルの修正・設定調整は親が直接書く**。**複数ファイル / 調査を伴う / 途中で判断が要る / 結果を読んで次を決める作業は subagent (`isolation: "worktree"`) に出す** — 出す理由は独立性ではなく**親のコンテキストの経済**(実装者とレビュアーの分離は Codex が担う / ADR 0035 D4)。**指示を一度で言い切れて成果が PR / Issue に残る作業は子セッション (`create_session`) に出してよい** (2026-08-12 に起動可能を実測。ADR 0033 の「承認ゲートで弾かれる」前提は解消)。ただし**子との会話は片道 約1分**かかる — `send_message` / `list_events` はこの環境に無く、メッセージは `create_trigger` + `run_once_at` を相手セッションに bind して送る ([ADR 0048](docs/adr/0048-child-sessions-are-usable-again-with-a-one-way-poke-channel.md) D6 / 手順は [Runbook](docs/runbooks/child-sessions.md))。**実測できているのは親 → 子の向きだけ。子 → 親の報告は Issue コメントを既定にする** (逆向きの Routine 経路は未検証 / ADR 0048 D7)。**往復が 1 回増えるごとに 1 分待つので、短い往復を繰り返す作業は子に出さず subagent にする**。**user にクリックを肩代わりさせない**。指示文は起票パケットの要件 (対象 Issue / 完遂条件 / **触ってはいけないファイル境界** / CLAUDE.md 参照) をそのまま満たし、子に出すときは「詰まったら Issue にコメントを残して終了する」も必ず書く
- **実務を回す Routine は web UI から作る。エージェントの `create_trigger` はセッション間メッセージ専用** (2026-08-12 PO 裁定 / 実測) — `create_trigger` には **`source_url` を渡す口が無く、作られた Routine は `sources` を持たない**。発火したセッションはリポジトリを掴めず、モデルも指定できず (opus ではなく sonnet に落ちる)、MCP connector も付かない。**仕事をさせる Routine としては使いものにならない**。実測で 2 本 (UX 採点 / PR レビュー代役 judge) がいずれも空撃ちになり、削除した。使ってよいのは [ADR 0048](docs/adr/0048-child-sessions-are-usable-again-with-a-one-way-poke-channel.md) D6 の用途 — **既にリポジトリを持っている相手セッションへ `run_once_at` でメッセージを届ける**経路に限る (この用途では相手側に repo があるので成立する)。なお **`update_trigger` は「自分が作った Routine」しか通らない** — PO が web UI で作った Routine (`created_via: http_api`) はエージェントから読めても書けない
- **子セッションを起こすときの必須事項** ([ADR 0048](docs/adr/0048-child-sessions-are-usable-again-with-a-one-way-poke-channel.md) D3〜D5) — `create_session` に **`source_url` と `source_revision` を必ず渡す** (環境は継承されるが**リポジトリは継承されない**。省略すると空の作業ディレクトリで止まる)。使い終わったら `archive_session` する。子が止まったら `get_session` の `status_category` を見る — `need_input` なら権限待ちなので、`needs_action` のツール名を `.claude/settings.json` の `allow` に足す (その場しのぎで user に承認させない。ただし **`deny` に載っているツールは足さない** — 塞いだのは意図なので、子には commit + PR に切り替えさせる)。**ただし allow の追加は起動済みの子には効かない** — 子は clone した時点のブランチの設定を読むので、**足したうえで子を起こし直す** (設定を main に入れるか、修正済みブランチを `source_revision` に指定する)。**MCP サーバ名は対話セッションと子セッションで違う** (`Claude_Code_Remote` / UUID `bf7c680d-...`) ので、allow は**両方の名前**を書く
