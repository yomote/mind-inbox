...
- **user への非同期の依頼は `needs-human` Issue に積む** ([ADR 0017](docs/adr/0017-needs-human-queue-and-po-inbox.md)) — チャットで頼むだけの依頼はセッションと共に消える。ラベル `needs-human` + タイトル接頭辞 `[needs-human]`、本文冒頭に種別 (判断/作業/確認) と「放置されると何が止まるか」1 文。ループを止める依頼は `P1`。Proposed ADR 等のミラー Issue は作らない
- **「私待ちのやつある?」には `po-inbox` skill** — needs-human Issue / Open PR / Proposed ADR / debrief の溜まり / journal 持ち越しをライブ集計し、優先度順 + 次の一手 1 つで出す
