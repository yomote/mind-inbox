...
| ルール | 性質 |
| --- | --- |
| 文分割 | 分割して連結すると**元の文字列に戻る** |
| 併合 | 同じ困りごとを 2 回抽出しても Problem の総数は増えない |
| 言及回数 | `mentionCount` は常に `mentions` の長さと等しい |
| 状態遷移 | 解決済みでも、再言及したら必ず `open` に戻る |
| タイトル導出 | 常に 26 文字以下で、空にならない |

道具は TypeScript が `fast-check`、Python が `hypothesis`。**まず文分割の 1 本**から入る
(往復の性質が最も書きやすい)。フロントエンドへの適用可否は未調査。

### 未着手 (この節を消すための残作業)

- BFF のドメインルールを `domain/` に切り出す
- `L3 mock E2E` 9 本の廃止と、旧称の一括置換
- §1 以降を上記 4 層で書き直す
- プロパティベーステストを 1 本入れて、有効性を確かめる
