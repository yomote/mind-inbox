...
## 2026-08-17 — debrief (+ #486 design-gate)

- **対象**: 2026-08-15 以降のマージ PR 9 本 — [#471](https://github.com/yomote/mind-inbox/pull/471) (mgmt-bootstrap キット) / [#475](https://github.com/yomote/mind-inbox/pull/475) / [#491](https://github.com/yomote/mind-inbox/pull/491) / [#492](https://github.com/yomote/mind-inbox/pull/492) (握り潰し撲滅 3 連) / [#469](https://github.com/yomote/mind-inbox/pull/469) / [#468](https://github.com/yomote/mind-inbox/pull/468) (CI 機構) / [#494](https://github.com/yomote/mind-inbox/pull/494) / [#484](https://github.com/yomote/mind-inbox/pull/484) / [#477](https://github.com/yomote/mind-inbox/pull/477) (小粒)。Proposed ADR は 0 件。あわせて #486 の design-gate と裁定キュー 5 件
- **決定** (PO 裁定 5 件): ① [#82](https://github.com/yomote/mind-inbox/issues/82) dev で `LLM_EXPOSED_TOOLS=offer_choices` の観測実験を実施 (Go) / ② [#410](https://github.com/yomote/mind-inbox/issues/410)/#409 週次 AI 審査は**経路 B (当番 tick に統合)** — 日曜 00 UTC 台の tick が debt-reviewer を回す / ③ SK 互換分岐 (history.py) は条件満了 (8/22 頃) 後に削除レーンへ ([#502](https://github.com/yomote/mind-inbox/issues/502)) / ④ [#486](https://github.com/yomote/mind-inbox/issues/486) **会話履歴の窓を design-gate 承認** — 窓方式 (ターン境界 / system 常時保持 / 件数+文字数 / converse と /approve の両経路 / save も刈る)。要約コンパクションは [#501](https://github.com/yomote/mind-inbox/issues/501) に分離 / ⑤ 当番 tick の backlog 自走投入は **rubric 強化 ([#503](https://github.com/yomote/mind-inbox/issues/503)) とセットで有効化** (rubric 着地が先行条件)
- **学びメモ**: A (#471) の逆質問 2 問は両方「分からん」→ 再解説で腹落ち — (1) 「取得の失敗」と「取得成功のうえで無い」の区別 (前者は落とす・後者は正常)、(2) code 変換 API は pem を HTTP 応答に同梱するため「秘密は応答に乗せない」設計と仕様レベルで両立不可。B の逆質問 (#475 の前後で何が変わったか) は「失敗したら落ちる」で正解 — 失敗の頻度でなく**嘘の成功だけが消えた**、が本質
- **特記**: 同日に mgmt-bootstrap キットの初回実実行 (live verification) でバグ 3 件 ([#497](https://github.com/yomote/mind-inbox/issues/497) / [#499](https://github.com/yomote/mind-inbox/issues/499))。全件が**外部システム境界** (GitHub manifest flow の意味論 / Azure RBAC 前提 / az 応答の形) で、スタブ 21 本 + 二重審査は**内部整合しか守っていなかった**。教訓の機構化が #503 (rubric に「外部境界の裏取り」観点)。PO は一時「新規レーン投入停止・成果物見直し」を指示し、本デブリーフがその見直しの場を兼ねた
- **持ち越し**: **未解説のマージ PR 8 本** (前回マーカー 2026-08-15 の同日 18:24 UTC 以降にマージ: #454 / #458 / #460 / #461 / #462 / #463 / #464 / #465) — 日付単位マーカーの取りこぼし (PR #504 の Codex 指摘で発覚)。次回デブリーフで扱う / #486 実装レーン (明日以降) / #82 実験 / 8/22 の #502 / #503 → tick 自走の有効化 / [#500](https://github.com/yomote/mind-inbox/issues/500) mgmt CD 化 (design-gate 済み・実装待ち) / PO 残作業: bootstrap.sh の完走 (検証 5b 修正後の再実行) とアイコン取得スクリプト実行
...
- **決定** (4 件):
  ...
  - **ADR 0056 を Accepted、あわせて読み替えの 1 行を追記** — 層の軸は「消えると困るか」ではなく「運用のためか / アプリそのものか」。**この Accept をもって [ADR 0046](../adr/0046-environment-rebuildable-from-declaration.md) D1 の supersede が発効**した (2026-08-14 の裁定では「Accept 時に確定」として保留していた)。追記した読み替えは、**0046 本文の「持続層にバックアップと GPG 秘密鍵が置かれる」を「管理系 RG のバックアップ Storage と、同 RG の Key Vault にある非エクスポートの RSA 鍵オブジェクト」と読む**というもの — 層の呼び名は 0056 D1 が、鍵の方式は [2026-08-12 の裁定記録](../adr/archive/operations/e2e-artifacts-are-secret-by-default.md) D5 の改訂 (GPG → 封筒暗号 + Key Vault 内復号) がそれぞれ置き換えており、**2 本の改訂が同じ 1 文に効いていて読み手が迷う**ため。**バックアップはバックアップのまま**で、置き換わるのは呼び名と鍵の方式の 2 点だけ。過去 ADR の本文は書き換えず、遷移時の PO 承認済み追記として **0056 側にのみ**置いた (0046 側は Status 行と既存注記の状態更新にとどめる — Accepted ADR に足してよいのは Status の範囲だけ)
...
- **持ち越し**: **バックアップ / 復元の往復 1 回** (ADR 0056 D2 / 0046 D9) — 通るまで `data-restore-unproven` の一律拒否は解けない (2026-08-14 から継続)。**[PR #342](https://github.com/yomote/mind-inbox/pull/342) の作り直し** — 内容を `dispatch` skill へ入れ、0049 を採らないことの後始末 (PR の close / `retired-numbers.txt` の扱い) は別セッション。(なお [`docs/adr/archive/operations/e2e-artifacts-are-secret-by-default.md`](../adr/archive/operations/e2e-artifacts-are-secret-by-default.md) の「ADR 0056 D1 (Proposed)」2 箇所は、**退避時点の凍結スナップショットではなく [#332](https://github.com/yomote/mind-inbox/pull/332) で後から足した現行案内**なので凍結規約の対象外、という窓口 PM 裁定を受けて追随させた。**凍結対象 — 冒頭の `Status:` 行と裁定記録の本文 — には触れていない**。`/adr` skill の「archive を編集しない」は**退避時点の記録を守る規約であって、後から足した現行案内まで凍らせるものではない**、という線引きがここで 1 つ決まった)

## 2026-08-15 — PO 裁定 (窓口 PM の委任境界)

- **対象**: 窓口 PM が PO に確認を取る範囲。正典は [`docs/team.md`](../team.md) の「委任の境界」(§2 窓口 PM の下)
- **決定**: **既定を「聞かずに決めて進め、事後に報告する」に振り切る。** PO の指示は「自律的にしてほしい。もちろん勝手にやるとめちゃめちゃまずいときは聞いてもらって全然いいんだけど、どんどんプロアクティブにやってほしい」。線引きは 3 区分 — **① 聞かずに決める (可逆な運用判断すべて**: Issue の整理・クローズ / 実装方式 / 着手順・並行度 / レビュー指摘への裁定 / PR の受け入れ / 規約文書の精緻化 / 承認済み設計の中の詳細判断**)** / **② PO に上げる** (不可逆・高コスト / 北極星 / PO 名義が要るもの = ADR の Accept・ux-rubric の改定。**対話をブロックせず、案 + 推奨つきでまとめて出す**) / **③ 即時に聞く** (取り返しがつかない事故になりうると PM が判断したとき。**この判断だけは保守側に倒す**)。①は**巻き戻せる形** (何を決めたか・なぜかを Issue / PR に残す) で進め、PO はいつでも覆せる
- **置き場の判断**: **ADR にしない** — システムアーキテクチャではなく運用・プロセスの決め事 ([`docs/adr/README.md`](../adr/README.md) / `/adr` skill の分類基準。運用系 ADR は `archive/` へ退避済みで現行ルールではない)。役割の正典である `docs/team.md` に置き、影響する 3 skill (`dispatch` = 窓口の「判断」の範囲 / `status` = 「🙋 あなたの番」に載せてよいもの / `merge` = 指摘への裁定と resolve の切り分け) を同じ PR で整合させた
- **学びメモ**: 自律の拡大と「裁定 3 件」([`streams.json`](../../cicd/scripts/status-page/streams.json) の `decision_batch`) は**対立しない — 強化される**。①を「🙋 あなたの番」に積むと 3 枠を食い潰し、本当にレーンを止めているものが 4 件目に落ちて「寝かせ中 n 件」に沈む。委任が広がるほど、この枠を②だけに保つ意味が強くなる
- **特記**: **「念のため聞く」は安全側ではない**という反転がこの裁定の核。窓口 PM が守る資源は PO の時間なので、確認を取る行為自体がその資源を削る。既存の「マージの常設承認」(`merge` skill / 2026-08-11 の「毎回言うのが面倒なので全セッションに効かせてほしい」) と同じ形の要求が、運用判断の全域に一般化された
- **持ち越し**: **③ (即時に聞いた) の回数をデブリーフで数える** — 増えているなら境界がずれているか、②に落とせるものを③に上げている。境界を動かせるのは PO 裁定だけ
