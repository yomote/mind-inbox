...
- **永続化は検証できない** (#165)。BFF は in-memory のままなので、
  「Functions がリサイクルしても困りごとが残るか」は別途 L4 で見る必要がある。
  **この層が緑でも実環境でデータが消える状態は続く。** 永続化の方針自体は
  design-gate #5 で決着済み (Cosmos DB 1 本に寄せ、BFF の内側に置く) なので、
  実装が入った時点でこの層の in-memory 前提 (spec の直列実行・題材トピック分割) を見直す
