...
- **D8 起動経路として専用の PR レビュー Routine を置く** (Negative Consequences の「自動起動が無く、呼ばれなければ沈黙する」への対処)。
  ...
  - **⚠️ Routine の登録はエージェントからはできない (2026-08-12 実測で確定)** — `create_trigger` は `source_url` / `repositories` を**黙って破棄**し、`create_session(source_url=…)` で作った repo 付きセッションに `persistent_session_id` で束ねても、**発火は束ねた先を起こさず新品セッション (sources 無し) を作る**。リポジトリの無い発火セッションは rubric も judge 定義も読めず、5 分動いて痕跡ゼロで終わる (2 回実測)。よって登録は web UI の手作業で、[#90](https://github.com/yomote/mind-inbox/issues/90) / [#156](https://github.com/yomote/mind-inbox/issues/156) と同型の needs-human が**発生する**。手順は [Runbook](../runbooks/review-agents.md)。**cron ではなく GitHub trigger (`pull_request` opened/synchronize) で登録する** — イベント駆動になり「最悪 6 時間遅れる」が消える
