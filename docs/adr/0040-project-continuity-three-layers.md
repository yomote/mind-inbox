...
- Status: Accepted (briefing #8 2026-08-11 で PO 承認) / D2 の頻度と作成主体を 2026-08-12 に運用改訂 (下記)
- Date: 2026-08-11
- Deciders: PO (yomote) / PM セッション

> **運用改訂 (2026-08-12 / PO 対話にて決定)** — 当番 PM の頻度を **1 日 3 回から 6 時間おき (1 日 4 回) へ**変更した。あわせて、同日の実測で `create_trigger` が承認ゲートを通ることが分かったため、Routine の実体を **web UI 作成版からエージェント作成版へ張り替えた**。詳細は D2。
...
**Option 1 (3 層構造) を採用。** 2026-08-11 に PO が選択肢形式で承認 (頻度: 1 日 3 回 / 通知: GitHub + push)。頻度は 2026-08-12 に **6 時間おき (1 日 4 回)** へ改訂 (D2)。
...
### D2 — 当番 PM は Routine (毎回新セッション・6 時間おき)

- Routine が **6 時間おき (UTC 00 / 06 / 12 / 18 = JST 09:00 / 15:00 / 21:00 / 03:00) に毎回新しいセッションを起こす** (`create_new_session_on_fire`)。どのセッションの生死にも依存しない
- **頻度は 2026-08-12 に 1 日 3 回 → 6 時間おきへ改訂** (PO 対話にて決定)。当初案の JST 08:00 / 13:00 / 18:00 は日中に寄っており夜間が 14 時間空いていた。等間隔にすることで最大空白が 6 時間に揃い、「夜のうちに詰まって朝まで放置」が構造的に消える。cron 式は `0 0,6,12,18 * * *` (UTC 指定・毎日)
...
- **Routine の実体はエージェント作成版に張り替えた (2026-08-12)**。Context の 4 番目「`create_trigger` は承認ゲートで停止する」は同日の実測で**覆った** — `create_trigger` は通り、弾かれたのは `fire_trigger` (即時試射) の方だった。承認ゲートは MCP ツール一律ではなく個別に効くため、「エージェントは Routine を作れない」と一般化しないこと
- ただし **web UI で作られた Routine はエージェントから更新できない** (`update_trigger` が `created_via: http_api` を拒否する)。頻度変更のたびに人間の操作を挟まないために、実体をエージェント作成版へ移した。旧 web UI 版の削除だけは PO の操作 (needs-human)
- **登録された頻度が ADR の記述と食い違っていた事故がある** — 本 ADR は当初 1 日 3 回 (JST 08:00/13:00/18:00) と書いていたが、実際に web UI へ登録されていたのは 18:00 JST の 1 日 1 回だけだった (2026-08-12 に発覚)。Routine の実体はリポジトリから読めないので、**頻度を変えたら ADR・watchers.json・実体の 3 点を必ず同時に合わせる**
...
- セッション起動コストが 1 日 4 回分増える (足りなければ頻度を上げる方が、常駐より経済的という判断)
