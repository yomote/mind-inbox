...
- **type**: 既定は `claude/` (Claude Code が実装したもの — 仕様の正式 type)。Codex が書く場合は `codex/`、人間 (PO) が手で切る場合は `feature/` `fix/` `chore/` `docs/` などの用途 type
...
- **例外**: Issue を立てるほどでない作業 (typo 修正 / 依存更新 / 実験ブランチ) は `chore/<slug>` `docs/<slug>` `test/<slug>` など**用途 type + 番号なし**にする。`dependabot/*` は Dependabot が生成する名前をそのまま使う (このリポジトリ側で変えられない)
