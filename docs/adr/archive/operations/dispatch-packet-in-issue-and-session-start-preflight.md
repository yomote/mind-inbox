...
#### 追試 (2026-08-11): 「名前不一致説」の実験は一度も成立していなかった

上の実証には反論があった (#164): 当時 allow に書いた文字列 `mcp__claude-code-remote` は実際の
サーバー登録名 `mcp__bf7c680d-5fdc-5ef4-b4a0-abadb619bf0a` と一致しておらず、**名前が合っていれば
通った可能性が残る**。briefing #4 で「正しい名前を settings.json に入れた」とされたが、2026-08-11 の
新セッションで確認すると **その変更は commit されておらず、コンテナと共に消えていた** (クラウド
セッションでは commit しない変更は残らない — 本 ADR の伏在条件そのもの)。同日の `list_triggers`
呼び出しは allow 行なしの状態で `-32003` — これは当然の結果であり、**判定材料にならない**。

今回、正しい登録名の read-only 1 本 (`…__list_triggers`) を allow に入れて **commit した**。
判定は次の新セッション冒頭で #164 の表に従って行う。なお、この実験がどちらに転んでも
「緑になったらマージ」の完遂は GitHub の auto-merge (サーバー側 / #234) が主経路であり、
セッション内の定期実行 (CronCreate) に完遂を依存させない — 2026-08-10 に PR #230 が
全 check 緑のまま一晩放置された実害による。
