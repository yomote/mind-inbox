...
| --- | --- | --- | --- |
| **SQL DB** | **S0（固定 DTU）** | **常時課金（月 ¥2,000 前後）** | **サーバーレス（GP_S）+ 自動一時停止**へ → compute ¥0・ストレージのみ数十円/月 |
...
→ **SQL を潰せば、待機は「小額固定（SWA Standard + ACR Basic ≒ 月 ¥2,000 前後）」だけ**に収まり、変動する高額コスト（SQL 常時 compute）は消える。

## Considered Options

- Option A: **常設・待機最小コスト + main マージ自動デプロイ**（SQL サーバーレス化 + Container Apps 環境統合 + image 事前ビルド + scale-to-zero 維持）
...
「本物をぱっと触りたい」に唯一まっすぐ応えるのが Option A。待機コストの主因（SQL S0 常時 compute）をサーバーレス自動一時停止で消せると判明したため、0009 が Option B（自動デプロイ = 常時起動）を却下した根拠（継続課金）が成立しなくなった。scale-to-zero（0002）と OIDC（0006/0009）はそのまま活き、既存 IaC/スクリプトを唯一の真実として再利用する。
...
- 待機時の高額変動費（SQL 常時 compute）が消え、残るのは小額固定のみ
- 既存 OIDC / スクリプト / 2-phase Bicep を流用（二重管理しない）

### Negative Consequences

- **SQL サーバーレスのコールドスタート**：自動一時停止からの再開に初回数秒〜十数秒（体験に許容範囲だが「一発目だけ遅い」）
...
- Good, because SQL サーバーレス化で待機の高額変動費が消える
