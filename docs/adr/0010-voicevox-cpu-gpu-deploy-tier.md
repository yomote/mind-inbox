...
オンデマンド CD（ADR 0009）で「使う時だけ立てて潰す」を実現したが、`up` に **~20〜30分**かかり、体験として「触りたくなってから待たされる」= オンデマンドの意味が薄い。
...
個人開発・デモ用途では **CPU で十分**（TTS が 1〜3 秒とやや遅い程度）。速く・安く立てられるモードを既定にしたい。一方で GPU（喋りが速い）も性能検証時に選びたい。
...
- `up` が **~20〜30分 → 数分〜10分**（GPU 環境 + 巨大イメージ + SQL を回避）
- 撤収時 ¥0 / GPU 課金なしが既定
