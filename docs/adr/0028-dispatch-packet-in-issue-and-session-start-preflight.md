...
>
> | 確認項目 | 本 ADR 起案時の実測 | briefing #4 セッションでの実測 |
> | --- | --- | --- |
> | `list_triggers` (読み取り専用) | `-32003 requires approval` | **成功** (Routine 6 件を取得) |
> | `create_trigger` (書き込み) | `-32003 requires approval` | `-32003 requires approval` (**同じく失敗**) |
> | 稼働中 Routine の作成経路 | エージェントは作成不可と判断 | `cd-watchdog` / `ux-judge` とも `created_via: meta_mcp` = **エージェントが登録済み** |
>
> したがって、誤っていたのは結論だけでなく**推論の形**だった。本 ADR は「読み取り専用も同じエラーになる」ことから**サーバー単位の遮断**と推論したが、briefing #4 では読み取りが通り書き込みだけが落ちた。**サーバー単位ではない。**可否は「セッション × ツール種別」で決まる。
>
> 現時点で実測に合う言い方は次のとおり:
>
> - **書き込み系は、少なくとも本 ADR 起案セッションと briefing #4 セッションの両方で塞がれていた** — したがって「エージェントは常に Routine を登録できる」も誤り
> - **しかし `created_via: meta_mcp` の Routine が現に 3 本動いている以上、書き込みが通るセッションは存在する** — したがって「恒久的な運用形態」も誤り
> - 何がこの差を生むのか (permission mode / 起動経路 / 承認済みツールの引き継ぎ) は**未解明**。分かるまでは「可否は不定。通らない前提で設計し、通ったら儲けもの」として扱う
>
