# 0046. セキュリティ対策を「無料枠優先 + 責任分担が重ならない層」で段階導入する
...
1. **会話セッションに所有者を持たせる** — `sessions` に userId 次元が無い問題。**マルチユーザー化の必須ブロッカー**であり、データが増えるほど移行コストが上がるので早い方が安い。partition key を変えるなら **Cosmos のコンテナ再作成が要る = 不可逆**。design-gate 必須
2. **HITL を実体化する** — 承認文面に**ツール引数を出す** (現状はツール名のみ)、承認 UI の実装、`/approve` の所有者チェックと監査記録。加えて `get_inbox_stats(user_id)` のように **user_id をモデル出力から受け取る**設計を、コンテキスト由来に変える。**ツールが実体を持つ前に**やる — 実体化した瞬間にプロンプト 1 通で IDOR になる
3. **指示とユーザー入力の境界を作る** — ツール結果 / RAG コンテキストを system ロールに入れない。間接インジェクションの主経路
4. **機密の出口を塞ぐ** — 抽出結果の要約が ERROR ログに全文 / 例外文字列がブラウザまで素通し / Azure OpenAI の abuse monitoring オプトアウト
5. **`x-ms-client-principal` の前提を実測する** — 「EasyAuth が inbound の同名ヘッダを上書きするから安全」というコメントの前提を確かめるテストもスモークも無い。しかも欠損時は fail-open で `"local"` に落ちる。**偽造ヘッダを実環境に投げて弾かれることを smoke-test に足す** ([ADR 0018](0018-runtime-verification-in-the-loop.md))
6. **AI ツール権限の不変条件テスト** — 「副作用のあるツールに `requiresApproval` が付いているか」を機械で検査する。**LLM 不要・決定的・今日書ける**。プロンプトやモデルを変えても壊れない回帰の土台 (Issue #313 の「AI 固有の攻撃パターンをテストとして蓄積する」の最初の一歩)

### Phase 3 — 余力が出たら

- **zizmor** (workflow の静的解析) — workflow 16 本 + OIDC + `packages:write` があるので費用対効果は高い。Phase 0 の SHA 固定が済んだ後の再発防止として
- **診断ログ / 監査ログ** — 現状 Function App / Container Apps / Cosmos / Storage のいずれにも診断設定が無く、**Cosmos へのアクセス記録が存在しない**。機微データを扱う以上、事故が起きたとき「何が読まれたか」を答えられない
- **IaC スキャン** — **Checkov は使わない** (公式 Issue #7394 で、モジュール化された現実的な Bicep では信頼できないと認めている)。**PSRule for Azure** (Bicep ネイティブ) が正解。ただし初期ルールセットが騒がしいので、誤検知を抑える設定とセットでなければ入れない
- **Trivy** (コンテナ / ベースイメージ CVE)
- **CIEM** — Entra Permissions Management は **2025 年に retire 済み**。`az role assignment list` の自前棚卸しが現実解
- **DAST** — このリポジトリのエージェントセッションは egress ポリシーで Azure エンドポイントに到達できない (実測: CONNECT に 403)。**動的検査は必ず Actions 側から回す** ([ADR 0031](0031-agent-reaches-outside-via-github-actions.md) の原則がそのまま効く)
