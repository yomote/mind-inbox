...
| 側 | やること | 使うもの |
| --- | --- | --- |
| CI (暗号化) | ランダム AES 鍵でデータを暗号化 → その AES 鍵を Key Vault の RSA **公開鍵**で wrap → 両方を artifact に入れる | `openssl` (ランナーに同梱) + 公開鍵 |
| エージェント (復号) | wrap 済み AES 鍵を `az keyvault key decrypt` で開かせる → 返った AES 鍵でデータを復号 | device-code + `az` |

**独自アルゴリズムは作らない** — これは KMS を使うときの定石 (envelope encryption) で、標準の AES と RSA-OAEP を組み合わせるだけ。とはいえ**自前で組み合わせる以上、実装ミスの余地はある**ので、実装時に「暗号化 → 復号 → 元データと一致」を機械で確かめるテストを必ず置く。
