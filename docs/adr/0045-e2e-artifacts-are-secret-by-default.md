...
- **D5 秘密鍵は持続層 RG の Key Vault に「鍵オブジェクト」として非エクスポートで置き、復号は Key Vault の中で行う** (2026-08-12 の debrief で PO 裁定により改訂)。**エージェントは復号してよい。ただし鍵そのものは一度も Key Vault の外に出ない。**
  ...
  - **非エクスポート方式で消える穴 / 消えない穴** (2026-08-12 の Codex 指摘で 2 度訂正。**2 度とも「この方式で防げる範囲」を広く見積もりすぎていた**)
    ...
    - **したがって「侵害の影響が時間で閉じる」とは言えない。** 言えるのは「**侵害された時点で保持されていた artifact に限られ、鍵交換なしに将来の artifact まで読まれ続けることはない**」まで
...
**残る露出はセッション単位** — 認証済みセッションが侵害されれば、トークンの有効期間中は保持中の wrapped key を何度でも開けるうえ、**開いて返った AES 鍵はトークンが切れても失効しない** (D5 の「消えない」参照)。鍵が持ち出せる旧案との差は「**再利用可能な万能鍵が残らず、鍵交換なしに将来の artifact まで読まれ続けることはない**」ことで、ゼロではない。
...
- **露出はセッション単位で残り、開いた分は時間で消えない** — 非エクスポート方式でも、認証済みセッションが侵害されればトークン有効期間中は保持中の wrapped key を何度でも開ける。「1 artifact ごとに承認」ではない。さらに **`decrypt` が返す AES 鍵は平文なので、トークン失効後もその時点で開いた artifact は読まれ続ける** (2026-08-12 の Codex 指摘で 2 度訂正)。旧案との差は「**再利用可能な万能鍵が残らない**」ことに限られる
