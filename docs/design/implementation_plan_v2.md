...
- **要点**: SK と MAF の併存を許す過渡期を作らず、モジュール単位で置き換える。**推論モデル (gpt-5 系等) の制約 (temperature / max_tokens 非対応) への対応は移行元に存在しなかったため、本移行でモデル名分岐として新規実装する** (#55 の実装メモ由来。PR #85 レビュー指摘で実態に合わせて訂正)
