...
- **段2 グルーピング**: 意味類似で **自動グルーピング**し、ユーザーが**事後トリアージ**（分割 / 統合 / 棚卸し）で上書きできる 🔶（PagerDuty 式 = A 案。承認ゲートを挟まない代わりに編集権はトリアージで担保）。
...
| §9-3 | 同一性 / 再出現判定 | 意味類似で**自動グルーピング** + 事後トリアージ。原因差は Mention が運ぶ | 🔶 方式は要レビュー |
| §9-4 | ラベル体系 | 上位 = 固定 taxonomy（横断比較のため）/ 下位 = LLM 自由タグ（ハイブリッド） | 🔶 上位カテゴリ一覧は未定 |

---

## 8. 次のアクション

1. `use_cases.md` の UC-01 / UC-03 を「抽出（段1）→ グルーピング（段2）」分離に更新
2. **ADR** を起こす:（a）集約ルート Session → Problem（b）2層 Mention/Problem + 自動グルーピング方式
3. 🔶 を確定（グルーピング方式の最終承認・上位ラベル taxonomy）
4. `basic_design.md §4`（データモデル）/ §10（ロードマップ）を更新
