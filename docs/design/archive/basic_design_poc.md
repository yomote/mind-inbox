# Mind Inbox — 基本設計書（PoC フェーズ / archive）

作成: 2026-05-01 / 対象フェーズ: PoC（最低限の一気通貫疎通）

> **⚠️ 現行の設計をここから読まないこと。** PoC 当時 (2026-05) のスナップショットで、以下はすでに覆されている:
>
> - §4 データモデル — Session 中心 → **Problem 中心 2層** ([ADR 0007](../../adr/0007-problem-centric-two-layer-domain-model.md))
> - §6 音声入力 — Web Speech API → **サーバー STT (Azure Speech)** ([ADR 0023](../../adr/0023-server-stt-azure-speech-f0.md))
> - §5 / §6.2 応答経路 — 単発レスポンス → **SSE ストリーミング** ([ADR 0024](../../adr/0024-chat-streaming-via-sse-side-channel.md))
> - §7 ワークフロー — Semantic Kernel の手書き FSM → **MAF へ移行中** ([ADR 0016](../../adr/0016-ai-agent-orchestration-on-maf.md))
> - §3 API 設計 — 手書きの表 → **OpenAPI が生成物 / zod・pydantic が真実** ([`docs/api/`](../../api/README.md))
> - §10 拡張ロードマップ — [`implementation_plan_v2.md`](../implementation_plan_v2.md) が後継
>
> 現行の構造は [`docs/design/basic_design.md`](../basic_design.md) と [ルート README](../../../README.md)。
...
**文面はここに転記しない** — 真実は `apps/services/ai-agent/app/prompts/chat.py`。自律改善ループ (ADR 0022 / [ADR 0027](../../adr/archive/operations/ux-improvement-loop-ab-protocol-and-mutation-boundary.md)) がこのファイルを自動で書き換えるため、doc 側にコピーを置くと**改定のたびに静かに陳腐化する**。
...
const getBaseUrl = () =>
  import.meta.env.DEV ? (import.meta.env.VITE_BFF_BASE_URL ?? "http://localhost:7071") : "";
