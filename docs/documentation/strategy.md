...
| 実行状態 (計画・進捗) | **GitHub Issues (`stream:*` ラベル)**     | `/status` skill / status ページの戦況図 (**#289 で実装予定 — 現時点のページに 5 レーンは出ない**) / docs へのリンクのみ (設計内容は Issue に書かない, ADR 0011 / [0044](../adr/archive/operations/stream-lanes-as-the-project-map.md))。**Projects board は退役 — 再建しない** |
| コンセプト            | `docs/concept_deck.md`                    | (現状維持)                                                                                                                                                                                                                                                                     |
| 基本設計              | `docs/design/`                            | (現状維持)                                                                                                                                                                                                                                                                     |
| テスト戦略            | `docs/testing/strategy.md`                | (現状維持)                                                                                                                                                                                                                                                                     |
| レビュー観点          | **rubric (`.github/claude/*-rubric.md`)** | subagent 定義 (`.claude/agents/`) / Runbook。rubric の**導出元となる実測記録**は `docs/reviews/` に置き、書き換えない ([ADR 0052](../adr/archive/operations/codex-derived-review-rubric-and-stand-in-judge.md))                                                                |
...
docs/
  ...
  debrief/              # design-gate / debrief / briefing の累積ログ
    journal.md
    archive/            # 一過性のセッション記録
  reviews/              # レビューの実測記録 (rubric の導出元。書き換えない)
    README.md
    {name}-YYYY-MM-DD.md
  documentation/        # この戦略
    strategy.md
