...
- `claude` `codex` `feature` `feat` `fix` `bugfix` `hotfix` `chore` は Conventional Branch 1.1.0 の正式 type。**`docs` `test` は仕様に無い本リポジトリの追加**で、既存の `docs/` `test/` に合わせた。**既存の `ci/` `tooling/` `pm/` は今後使わず `chore/` に寄せる**。`ops/` も同様だが、**`ops/inspect` だけは 4 の予約名前空間** (OIDC 資格情報の subject) であり、寄せる・消すの対象ではない
...
- **したがって `claude/` `codex/` の prefix は「Issue 番号を持つ」ことのしるしになる**。機械判定を書くときは Verification の**完全パターン 3 本**を使う (prefix だけの部分一致は判定に使わない — Verification の注記のとおり、許可した形式を弾き、違反の形式を通す)
...
| `ops/inspect`                   | OIDC フェデレーション資格情報の subject (`repo:yomote/mind-inbox:ref:refs/heads/ops/inspect`) に固定され、`.github/workflows/ops-inspect.yml` が read-only の Azure login に使う ([`azure-oidc-cd-setup.md`](azure-oidc-cd-setup.md))。資格情報はワイルドカード指定できず、このブランチ 1 本に紐づく。**削除すると ops-inspect の Azure login が壊れる** (次に走らせるまで気づけない) | 現役                                                                                                                                                                                                                                      |
...
- **削除 (merged)**: PR が merged になった時点で head を削除する。GitHub の "Automatically delete head branches" (`delete_branch_on_merge`) で機構化できるが、**現在値は未取得** — 管理系 API が 403 で読めていない ([`cicd/github/terraform/unmanaged.tf`](../../cicd/github/terraform/unmanaged.tf) に「未取得」として記録済み)。**有効になっている前提で書かないこと**。実行経路は Steps の D — **有効化は web UI 操作なので `needs-human` Issue に積む**。有効化までは merged PR の head も削除されず残る (2026-08-15 実測: 直近マージ 15 PR のうち 13 本の head が origin に残存。消えていた 2 本は dependabot が自分で消したもの)。**merged かつ head が残っている PR は [#343](https://github.com/yomote/mind-inbox/issues/343) の掃除対象に含める**
...
### D. merged PR の head 削除の経路を作る (`delete_branch_on_merge`)

1. GitHub の "Automatically delete head branches" (`delete_branch_on_merge`) の**有効化を `needs-human` Issue に積む** — web UI 操作なのでエージェントは実施できない。現在値も未取得 (5)
2. 有効化されるまで merged PR の head は削除されず残る。**残った head は [#343](https://github.com/yomote/mind-inbox/issues/343) の掃除対象に含める** (C の 4 条件とは別枠 — merged は PR の状態で機械判定でき、30 日の待機も要らない)

## Verification

- [ ] **この文書の着地後に作成された、base が `main` の PR** の head 名が、下の**完全パターン**のいずれかに合致する。**どれにも合致しないものが 0 本**であること。base を `main` に限定するのは、リリース PR (`main → release`) の head が `main` で作業ブランチではなく、どのパターンにも合致しないため (限定しないと、リリース PR を 1 本開いた時点でこの条件が恒久的に満たせなくなる)
...
- **未検証: 「merged な PR の head が残っていない」は評価条件から外している** — 前提の `delete_branch_on_merge` の現在値が管理系 API 403 で未取得のため、「有効化後」を起点にした判定ができない。有効化 (Steps D) が確認できたらチェック項目に戻す。それまでに merged で残った head は [#343](https://github.com/yomote/mind-inbox/issues/343) の掃除対象
...
- まず区別する: **`ops/inspect` は残骸ではなく 4 の予約** (OIDC 資格情報の subject)。掃除候補にも改名対象にもしない
- 原因: type 表が閉じる前に切られたもの (2026-08-15 実測: `ci/lint-typecheck-build` / `tooling/pr-readiness` / `pm/verify-py314-384` の各 1 本。`ops/` は `ops/inspect` のみで、それ以外の残骸は 0 本)
