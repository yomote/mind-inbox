...
| ---------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 鍵がエクスポート不可         | `az keyvault key show --vault-name <Vault> -n e2e-artifacts --query attributes.exportable -o tsv` | **空** または **`false`** (エクスポート不可の鍵は `attributes` に `exportable` を持たないので**空が正常**。異常は明示的な `true` だけ / [#499](https://github.com/yomote/mind-inbox/issues/499)) |
