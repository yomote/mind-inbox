...
| ----------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------- |
| **機械計測** (レイテンシ統計 / 往復数 / 警告数) | [`.github/workflows/ux-eval.yml`](../../.github/workflows/ux-eval.yml)          | 毎朝、golden-path-monitor 完了直後 (workflow_run) | `kind: "ux-eval-mech"`    |
...
取れた分は追記したうえで、前回まで積めていたシナリオ集合 (直近 7 日の evals の実績) より
減っていれば ux-eval の run を赤にする** (#443 — 部分欠落を緑で通すと、封筒化失敗のように
monitor が緑のまま欠ける経路で 1 シナリオだけ蓄積が止まってもどこも赤くならない)。
飛ばしたシナリオ名は ux-eval のログに残り、プローブ自体の失敗は golden-path-monitor の
赤でも通報される。
...
- [ ] **人手を介さず** `evals/YYYY-MM.jsonl` に機械計測 (`ux-eval-mech`) の行が増えている — ux-eval.yml の run (golden-path-monitor 完了直後に起動) で確認。**未検証**
...
- 対処: 落ちたシナリオの E2E ログで壊れたホップを切り分ける。ux-eval は取れた分を積んだうえで、
  **前回まで積めていた集合より減っていれば赤になる** (#443)。欠落初日は前日分が「評価済み」として
  covered に数えられるため ux-eval はまだ緑のことがある — その朝は monitor の赤が知らせ、
  翌朝以降は ux-eval の赤が知らせる (二重の見張り)
...
- 原因: 26 時間以内のプローブ記録が `probes/` に無い (プローブ手前で fail / 追記失敗)、
  データブランチが無い (初期移行前)、追記 push の失敗、または**一部シナリオの蓄積停止**
  (前回まで積めていた集合より減った — exit 5。取れた分は追記済み / #443)。
  **古い記録・欠けた記録を静かに通さないための意図した赤** (ADR 0037)
...
- 原因: **表示は必ず「競合」だが、実際は競合とは限らない** (#476)。同時 push の競合の
  ほか、権限不足・認証切れ・保護ブランチ・pre-receive 拒否でも同じ文言が出る。書き込みは
  朝の 2 本 + PM tick で時間帯が分かれているので、真の競合は通常は起きない
- 対処: **まず push の生の stderr を見る**。`non-fast-forward` / `fetch first` なら本当の競合で、
  再実行すれば合流する。それ以外 (`permission denied` / `pre-receive hook declined` 等) は
  **再実行しても直らない** — 権限とブランチ保護を疑う
